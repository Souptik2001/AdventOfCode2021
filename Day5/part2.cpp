...
int diagonalLine(int x1, int x2, int y1, int y2, map<pair<int, int>, int> &cells){
    int ans = 0;
    bool dec = false;
    int x;
    int y;
...
int solve(vector<vector<int>> input){
  ...
  for(int i=0; i<input.size(); i++){
    ...
    }else if(abs(input[i][0]-input[i][2]) == abs(input[i][1]-input[i][3])){
      ans += diagonalLine(input[i][0], input[i][2], input[i][1], input[i][3], cells);
    }
  }
return ans;
}
